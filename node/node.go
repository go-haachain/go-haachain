...
func (n *Node) OpenDatabase(name string, cache, handles int) (haadb.Database, error) {
	if n.config.DataDir == "" {
		return haadb.NewMemDatabase()
	}
	return haadb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
