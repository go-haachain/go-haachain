...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (haadb.Database, error) {
	if ctx.config.DataDir == "" {
		return haadb.NewMemDatabase()
	}
	db, err := haadb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
