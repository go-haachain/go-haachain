...
// Package miner implements haachain block creation and mining.
package miner
...
type Miner struct {
	...
	canStart    int32 // can start indicates whhaaer we can start the mining operation
	shouldStart int32 // should start indicates whhaaer we should start after sync
}

func New(haa Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		haa:      haa,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, haa, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(haa.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) Start(coinbase common.Address) {
	atomic.StoreInt32(&self.shouldStart, 1)
	self.worker.sethaaerbase(coinbase)
...
func (self *Miner) Sethaaerbase(addr common.Address) {
	self.coinbase = addr
	self.worker.sethaaerbase(addr)
}
