...
var (
	...
	configPrefix   = []byte("haaereum-config-") // config prefix for the db
...
func WriteCanonicalHash(db haadb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db haadb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db haadb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db haadb.Putter, hash common.Hash) error {
...
func WriteTrieSyncProgress(db haadb.Putter, count uint64) error {
...
func WriteHeader(db haadb.Putter, header *types.Header) error {
...
func WriteBody(db haadb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db haadb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db haadb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db haadb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db haadb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db haadb.Putter, block *types.Block) error {
...
func WriteBloomBits(db haadb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db haadb.Database) haadb.Database {
	return haadb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db haadb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db haadb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db haadb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
