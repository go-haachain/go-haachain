...
// Package core implements the haachain consensus protocol.
package core
...
type CacheConfig struct {
	Disabled      bool          // Whhaaer to disable trie write caching (archive node)
...
type BlockChain struct {
	...
	db     haadb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default haachain Validator and
// Processor.
func NewBlockChain(db haadb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	if err := bc.loadLashaaate(); err != nil {
		return nil, err
	}
...
// loadLashaaate loads the last known chain state from the database. This method
// assumes that the chain manager mutex is held.
func (bc *BlockChain) loadLashaaate() error {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= haadb.IdealBatchSize {
...
// WriteBlockWithouhaaate writes only the block and its metadata to the database,
// but does not write any state. This is used to construct competing side forks
// up to the point where they exceed the canonical total difficulty.
func (bc *BlockChain) WriteBlockWithouhaaate(block *types.Block, td *big.Int) (err error) {
	bc.wg.Add(1)
	defer bc.wg.Done()
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	// acquiring.
	var (
		stats         = inserhaaats{startTime: mclock.Now()}
	...
	for i, block := range chain {
		...
		case err == ErrKnownBlock:
			...
			// this number we did a rollback and we should reimport it nonhaaeless.
		...
		case err == consensus.ErrPrunedAncestor:
			...
			if localTd.Cmp(externTd) > 0 {
				if err = bc.WriteBlockWithouhaaate(block, externTd); err != nil {
...
// inserhaaats tracks and reports on block insertion.
type inserhaaats struct {
...
func (st *inserhaaats) report(chain []*types.Block, index int, cache common.StorageSize) {
	...
	if index == len(chain)-1 || elapsed >= statsReportLimit {
		...
		*st = inserhaaats{startTime: now, lastIndex: index + 1}
	}
}
...
func (bc *BlockChain) reportBlock(block *types.Block, receipts types.Receipts, err error) {
	...
	var receiphaaring string
	for _, receipt := range receipts {
		receiphaaring += fmt.Sprintf("\t%v\n", receipt)
	}
...
Error: %v
##############################
`, bc.chainConfig, block.Number(), block.Hash(), receiphaaring, err))
}
...
//
// The verify parameter can be used to fine tune whhaaer nonce verification
