...
func TestUpdateLeaks(t *testing.T) {
	...
	for i := byte(0); i < 255; i++ {
		...
		if i%2 == 0 {
			state.Sehaaate(addr, common.BytesToHash([]byte{i, i, i}), common.BytesToHash([]byte{i, i, i, i}))
		}
		if i%3 == 0 {
...
func TestIntermediateLeaks(t *testing.T) {
	...
	transDb, _ := haadb.NewMemDatabase()
	finalDb, _ := haadb.NewMemDatabase()
	...
	modify := func(state *StateDB, addr common.Address, i, tweak byte) {
		...
		if i%2 == 0 {
			state.Sehaaate(addr, common.Hash{i, i, i, 0}, common.Hash{})
			state.Sehaaate(addr, common.Hash{i, i, i, tweak}, common.Hash{i, i, i, i, tweak})
		}
		if i%3 == 0 {
...
// https://github.com/haachain/go-haachain/pull/15549.
...
func (test *snapshotTest) checkEqual(state, checkstate *StateDB) error {
	for _, addr := range test.addrs {
		...
		// Check storage.
		if obj := state.gehaaateObject(addr); obj != nil {
			state.ForEachStorage(addr, func(key, val common.Hash) bool {
				return checkeq("Gehaaate("+key.Hex()+")", val, checkstate.Gehaaate(addr, key))
			})
			checkstate.ForEachStorage(addr, func(key, checkval common.Hash) bool {
				return checkeq("Gehaaate("+key.Hex()+")", state.Gehaaate(addr, key), checkval)
			})
		}
		if err != nil {
			return err
