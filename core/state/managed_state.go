...
// Sehaaate sets the backing layer of the managed state
func (ms *ManagedState) Sehaaate(statedb *StateDB) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
	ms.StateDB = statedb
}
...
// HasAccount returns whhaaer the given address is managed or not
...
func (ms *ManagedState) getAccount(addr common.Address) *account {
	if account, ok := ms.accounts[addr]; !ok {
		...
		so := ms.StateDB.gehaaateObject(addr)
