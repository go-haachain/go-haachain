...
type operation struct {
	...
	halts   bool // indicates whhaaer the operation should halt further execution
	jumps   bool // indicates whhaaer the program counter should not increment
	writes  bool // determines whhaaer this a state modifying operation
	valid   bool // indication whhaaer the retrieved operation is valid and known
	reverts bool // determines whhaaer the operation reverts state (implicitly halts)
	returns bool // determines whhaaer the operations sets the return data content
}
...
func NewConstantinopleInstructionSet() [256]operation {
	...
	instructionSet[SHL] = operation{
		execute:       opSHL,
		gasCost:       constGasFunc(GasFasteshaaep),
...
func NewFrontierInstructionSet() [256]operation {
	return [256]operation{
		...
		MUL: {
			execute:       opMul,
			gasCost:       constGasFunc(GasFashaaep),
			validateStack: makeStackFunc(2, 1),
			valid:         true,
		},
		SUB: {
		...
		BLOCKHASH: {
			execute:       opBlockhash,
			gasCost:       constGasFunc(GasExhaaep),
			validateStack: makeStackFunc(1, 1),
			valid:         true,
		},
		COINBASE: {
