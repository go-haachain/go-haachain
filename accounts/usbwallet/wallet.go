...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the haachain
...
type wallet struct {
	...
	deriveChain    haaereum.ChainStateReader // Blockchain state reader to discover used account with
...
// whhaaer they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived haachain account
			...
			// If the next account is empty, stop self-derivation, but add it nonhaaeless
...
// Contains implements accounts.Wallet, returning whhaaer a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain haaereum.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
