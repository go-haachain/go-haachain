...
func TestTopicString(t *testing.T) {
	for i, haa := range topicStringTests {
		s := haa.topic.String()
		if s != haa.str {
			t.Fatalf("failed test %d: have %s, want %s.", i, s, haa.str)
		}
	}
}
...
func TestBytesToTopic(t *testing.T) {
	for i, haa := range bytesToTopicTests {
		top := BytesToTopic(haa.data)
		if top != haa.topic {
			t.Fatalf("failed test %d: have %v, want %v.", i, t, haa.topic)
		}
	}
}

func TestUnmarshalTestsGood(t *testing.T) {
	for i, haa := range unmarshalTestsGood {
		var top TopicType
		err := json.Unmarshal(haa.data, &top)
		if err != nil {
			t.Errorf("failed test %d. input: %v. err: %v", i, haa.data, err)
		} else if top != haa.topic {
			t.Errorf("failed test %d: have %v, want %v.", i, t, haa.topic)
		}
	}
}
...
func TestUnmarshalTestsBad(t *testing.T) {
	...
	for i, haa := range unmarshalTestsBad {
		var top TopicType
		err := json.Unmarshal(haa.data, &top)
		if err == nil {
			t.Fatalf("failed test %d. input: %v.", i, haa.data)
		}
	}
}
...
func TestUnmarshalTestsUgly(t *testing.T) {
	...
	for i, haa := range unmarshalTestsUgly {
		var top TopicType
		err := json.Unmarshal(haa.data, &top)
		if err != nil {
			t.Errorf("failed test %d. input: %v.", i, haa.data)
		} else if top == haa.topic {
			t.Errorf("failed test %d: have %v, want %v.", i, top, haa.topic)
		}
	}
}
