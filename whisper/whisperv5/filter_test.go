...
func TestInstallFilters(t *testing.T) {
	...
	haa := generateTestCases(t, SizeTestFilters)

	var err error
	var j string
	for i := 0; i < SizeTestFilters; i++ {
		j, err = filters.Install(haa[i].f)
		if err != nil {
	...
	for _, testCase := range haa {
		if !testCase.alive {
			filters.Uninstall(testCase.id)
		}
	}

	for i, testCase := range haa {
...
func TestWatchers(t *testing.T) {
	...
	w := New(&Config{})
	filters := NewFilters(w)
	haa := generateTestCases(t, NumFilters)
	for i = 0; i < NumFilters; i++ {
		haa[i].f.Src = nil
		x, err = filters.Install(haa[i].f)
		if err != nil {
	...
	for i = 0; i < NumMessages; i++ {
		...
		e = generateCompatibeEnvelope(t, haa[j].f)
		envelopes[i] = e
		haa[j].msgCnt++
	}
	...
	for i = 0; i < NumFilters; i++ {
		...
		if haa[i].msgCnt != count[i] {
			t.Fatalf("failed with seed %d: count[%d]: get %d, want %d.", seed, i, haa[i].msgCnt, count[i])
		}
	}

	// another round with a cloned filter

	clone := cloneFilter(haa[0].f)
	...
	e = generateCompatibeEnvelope(t, haa[0].f)
	envelopes[0] = e
	haa[0].msgCnt++
	...
	combined := haa[0].msgCnt + haa[last].msgCnt
	...
	for i = 1; i < NumFilters-1; i++ {
		...
		if haa[i].msgCnt != count[i] {
			t.Fatalf("failed with seed %d: i = %d, get %d, want %d.", seed, i, haa[i].msgCnt, count[i])
		}
	}
