...
import (
	...
	"github.com/haachain/go-haachain/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Ghaa console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = haa.getBlock(haa.getTransaction(tx).blockHash);
  var genesis = haa.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.haa.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  haa.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
