...
type Fetcher struct {
	...
	fetchingHook       func([]common.Hash)     // Method to call upon starting a block (haa/61) or header (haa/62) fetch
	completingHook     func([]common.Hash)     // Method to call upon starting a block body fetch (haa/62)
	importedHook       func(*types.Block)      // Method to call upon successful block import (both haa/61 and haa/62)
}
...
func (f *Fetcher) loop() {
	...
	for {
		...
		case notification := <-f.notify:
			...
			if count > hashLimit {
				log.Debug("Peer exceeded ouhaaanding announces", "peer", notification.origin, "limit", hashLimit)
...
func (f *Fetcher) insert(peer string, block *types.Block) {
	...
	go func() {
		...
		default:
			// Somhaaing went very wrong, drop the peer
