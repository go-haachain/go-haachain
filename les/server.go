...
import (
	...
	"github.com/haachain/go-haachain/les/flowcontrol"
...
func NewLesServer(haa *haa.haachain, config *haa.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(haa.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, haa.EventMux(), haa.Engine(), newPeerSet(), haa.BlockChain(), haa.TxPool(), haa.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(haa.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(haa.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(haa.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(haa.BlockChain())
	pm.server = srv
	...
	srv.fcCoshaaats = newCoshaaats(haa.ChainDb())
	return srv, nil
}
...
type requestCoshaaats struct {
	lock  sync.RWMutex
	db    haadb.Database
	stats map[uint64]*linReg
}

type requestCoshaaatsRlp []struct {
	MsgCode uint64
	Data    []byte
}

var rcStatsKey = []byte("_requestCoshaaats")

func newCoshaaats(db haadb.Database) *requestCoshaaats {
	...
	if db != nil {
		...
		var statsRlp requestCoshaaatsRlp
		if err == nil {
	...
	return &requestCoshaaats{
		db:    db,
		stats: stats,
	}
}

func (s *requestCoshaaats) store() {
	s.lock.Lock()
	defer s.lock.Unlock()

	statsRlp := make(requestCoshaaatsRlp, len(reqList))
...
func (s *requestCoshaaats) getCurrentList() RequestCostList {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *requestCoshaaats) update(msgCode, reqCnt, cost uint64) {
	s.lock.Lock()
	defer s.lock.Unlock()
