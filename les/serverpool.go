...
const (
	...
	// inihaaatsWeight is used to initialize previously unknown peers with good
...
type serverPool struct {
	...
	adjushaaats          chan poolStatAdjust
...
func newServerPool(db haadb.Database, quit chan struct{}, wg *sync.WaitGroup) *serverPool {
	pool := &serverPool{
		db:           db,
		quit:         quit,
		wg:           wg,
		...
		adjushaaats:  make(chan poolStatAdjust, 100),
...
func (pool *serverPool) disconnect(entry *poolEntry) {
	...
	if entry.state == psRegistered {
		...
		if stopped {
			entry.connechaaats.add(1, connAdjust)
		} else {
			entry.connechaaats.add(connAdjust, 1)
		}
	}
...
func (pool *serverPool) adjustBlockDelay(entry *poolEntry, time time.Duration) {
	...
	pool.adjushaaats <- poolStatAdjust{pseBlockDelay, entry, time}
}
...
func (pool *serverPool) adjustResponseTime(entry *poolEntry, time time.Duration, timeout bool) {
	...
	if timeout {
		pool.adjushaaats <- poolStatAdjust{pseResponseTimeout, entry, time}
	} else {
		pool.adjushaaats <- poolStatAdjust{pseResponseTime, entry, time}
	}
}
	...
	for {
		...
		case adj := <-pool.adjushaaats:
			...
			case pseResponseTime:
				...
				adj.entry.timeouhaaats.add(0, 1)
			case pseResponseTimeout:
				adj.entry.timeouhaaats.add(1, 1)
			}
			pool.lock.Unlock()
...
func (pool *serverPool) findOrNewNode(id discover.NodeID, ip net.IP, port uint16) *poolEntry {
	...
	if entry == nil {
		...
		entry.connechaaats.add(1, inihaaatsWeight)
		entry.delayStats.add(0, inihaaatsWeight)
		entry.responseStats.add(0, inihaaatsWeight)
		entry.timeouhaaats.add(0, inihaaatsWeight)
	}
...
func (pool *serverPool) loadNodes() {
	...
	for _, e := range list {
		log.Debug("Loaded server stats", "id", e.id, "fails", e.lastConnected.fails,
			"conn", fmt.Sprintf("%v/%v", e.connechaaats.avg, e.connechaaats.weight),
			"delay", fmt.Sprintf("%v/%v", time.Duration(e.delayStats.avg), e.delayStats.weight),
			"response", fmt.Sprintf("%v/%v", time.Duration(e.responseStats.avg), e.responseStats.weight),
			"timeout", fmt.Sprintf("%v/%v", e.timeouhaaats.avg, e.timeouhaaats.weight))
		pool.entries[e.id] = e
...
type poolEntry struct {
	...
	connechaaats, delayStats    poolStats
	responseStats, timeouhaaats poolStats
	state                       int
...
func (e *poolEntry) EncodeRLP(w io.Writer) error {
	return rlp.Encode(w, []interface{}{e.id, e.lastConnected.ip, e.lastConnected.port, e.lastConnected.fails, &e.connechaaats, &e.delayStats, &e.responseStats, &e.timeouhaaats})
}
...
func (e *poolEntry) DecodeRLP(s *rlp.Stream) error {
	var entry struct {
		...
		Port                       uint16
		Fails                      uint
		CStat, DStat, RStat, haaat poolStats
	}
	...
	e.connechaaats = entry.CStat
	e.delayStats = entry.DStat
	e.responseStats = entry.RStat
	e.timeouhaaats = entry.haaat
...
func (e *knownEntry) Weight() int64 {
	...
	return int64(1000000000 * e.connechaaats.recentAvg() * math.Exp(-float64(e.lastConnected.fails)*failDropLn-e.responseStats.recentAvg()/float64(responseScoreTC)-e.delayStats.recentAvg()/float64(delayScoreTC)) * math.Pow(1-e.timeouhaaats.recentAvg(), timeoutPow))
}
