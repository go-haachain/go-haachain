...
import (
	...
	"github.com/haachain/go-haachain/internal/jsre"
	"github.com/haachain/go-haachain/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute haachain requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jhaa", struct{}{})

	jhaaObj, _ := c.jsre.Get("jhaa")
	jhaaObj.Object().Set("send", bridge.Send)
	jhaaObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jhaa);"); err != nil {
	...
	flatten := "var haa = web3.haa; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = haa.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jhaa.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jhaa.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jhaa.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jhaa.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jhaa.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines haa.getBalance(haa.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Ghaa instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Ghaa metadata
	fmt.Fprintf(c.printer, "Welcome to the Ghaa JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + haa.coinbase);
		console.log("at block: " + haa.blockNumber + " (" + new Date(1000 * haa.getBlock(haa.blockNumber).timestamp) + ")");
