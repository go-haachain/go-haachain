...
func newTester(t *testing.T, confOverride func(*haa.Config)) *tester {
	...
	// Create a networkless protocol stack and start an haachain service within
	...
	haaConf := &haa.Config{
		...
		haaerbase: common.HexToAddress(testAddress),
		haaash: ethash.Config{
			PowMode: ethash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(haaConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return haa.New(ctx, haaConf) }); err != nil {
		t.Fatalf("failed to register haachain protocol: %v", err)
	}
