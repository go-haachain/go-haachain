...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Ghaa.LightScryptN, Ghaa.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.haaereum", new NodeConfig());
			node.start();
			...
			haachainClient ec = node.gethaachainClient();
	...
	// signatures too. Regression test for go-haaereum issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altoghaaer
	...
	workspace, err := ioutil.TempDir("", "ghaa-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/haaereum/ghaatest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Ghaa and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.haaereum", "github.com/haachain/go-haachain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "ghaa.aar"), "ghaa.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "haaereum", "ghaatest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
