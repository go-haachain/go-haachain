...
// Package chequebook package wraps the 'chequebook' haachain smart contract.
//
// The functions in this package allow using chequebook for
// issuing, receiving, verifying cheques in haaer; (auto)cashing cheques in haaer
// as well as (auto)depositing haaer to the chequebook contract.
package chequebook
...
import (
	...
	"github.com/haachain/go-haachain/contracts/chequebook/contract"
	"github.com/haachain/go-haachain/core/types"
	"github.com/haachain/go-haachain/crypto"
	"github.com/haachain/go-haachain/log"
	"github.com/haachain/go-haachain/swarm/services/swap/swap"
)
...
// * depositing haaer to the chequebook
// * watching incoming haaer
...
func (self *Chequebook) UnmarshalJSON(data []byte) error {
	...
	_, ok := self.balance.Sehaaring(file.Balance, 10)
	if !ok {
	...
	for addr, sent := range file.Sent {
		self.sent[common.HexToAddress(addr)], ok = new(big.Int).Sehaaring(sent, 10)
		if !ok {
...
// Cash cashes the cheque by sending an haachain transaction.
