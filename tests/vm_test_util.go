...
// See https://github.com/haaereum/tests/wiki/VM-Tests for the test format specification.
type VMTest struct {
	json vmJSON
}
...
type vmJSON struct {
	...
	PoshaaateRoot common.Hash           `json:"poshaaateRoot"`
}
...
func (t *VMTest) Run(vmconfig vm.Config) error {
	...
	for addr, account := range t.json.Post {
		for k, wantV := range account.Storage {
			if haveV := statedb.Gehaaate(addr, k); haveV != wantV {
				return fmt.Errorf("wrong storage value at %x:\n  got  %x\n  want %x", k, haveV, wantV)
			}
		}
	}
	// if root := statedb.IntermediateRoot(false); root != t.json.PoshaaateRoot {
	// 	return fmt.Errorf("post state root mismatch, got %x, want %x", root, t.json.PoshaaateRoot)
	// }
