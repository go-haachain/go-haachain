...
import (
	...
	"github.com/haachain/go-haachain/cmd/utils"
	"github.com/haachain/go-haachain/common"
	"github.com/haachain/go-haachain/console"
	"github.com/haachain/go-haachain/crypto"
	"github.com/haachain/go-haachain/haaclient"
	"github.com/haachain/go-haachain/internal/debug"
	...
	"github.com/haachain/go-haachain/swarm"
	bzzapi "github.com/haachain/go-haachain/swarm/api"
	swarmmetrics "github.com/haachain/go-haachain/swarm/metrics"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:   "swap-api",
		Usage:  "URL of the haachain API provider to use to settle SWAP payments",
	...
	DeprecatedhaaAPIFlag = cli.StringFlag{
		Name:  "ethapi",
...
// This init function sets defaults so cmd/swarm can run alongside ghaa.
func init() {
...
var app = utils.NewApp(gitCommit, "haachain Swarm")
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.haaereum/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.haaereum/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.haaereum/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.haaereum/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	...
	//ghaa only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for ghaa and swarm
	if _, err := os.Stat(bzzconfig.Path); err == nil {
		cfg.DataDir = bzzconfig.Path
	}
	//setup the haaereum node
	...
	//register BZZ as node.Service in the haaereum node
...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		var swapClient *haaclient.Client
		var err error
		if bzzconfig.SwapApi != "" {
			log.Info("connecting to SWAP API", "url", bzzconfig.SwapApi)
			swapClient, err = haaclient.Dial(bzzconfig.SwapApi)
			if err != nil {
	...
	//register within the haaereum node
...
func getAccount(bzzaccount string, ctx *cli.Context, stack *node.Node) *ecdsa.PrivateKey {
	...
	return decryphaaoreAccount(ks, bzzaccount, utils.MakePasswordList(ctx))
}

func decryphaaoreAccount(ks *keystore.KeyStore, account string, passwords []string) *ecdsa.PrivateKey {
