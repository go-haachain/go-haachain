...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.haastats == "" {
		log.Error("No haastats server configured")
		return
	}
	...
	if infos.datadir == "" {
		...
		infos.datadir = w.readDefaulhaaring(infos.datadir)
	}
	if w.conf.Genesis.Config.haaash != nil && !boot {
		fmt.Println()
		if infos.ethashdir == "" {
			...
			infos.ethashdir = w.readDefaulhaaring(infos.ethashdir)
		}
	}
	...
	if infos.haastats == "" {
		...
		infos.haastats = w.readString() + ":" + w.conf.haastats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.haastats)
		infos.haastats = w.readDefaulhaaring(infos.haastats) + ":" + w.conf.haastats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.haaash != nil {
			// haaash based miners only need an haaerbase to mine against
			fmt.Println()
			if infos.haaerbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.haaerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.haaerbase)
				infos.haaerbase = w.readDefaultAddress(common.HexToAddress(infos.haaerbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy haachain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
