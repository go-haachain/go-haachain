...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for haastats availability")
	if infos, err := checkhaastats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["haastats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["haastats"] = infos.Report()
		haastats = infos.config
	}
