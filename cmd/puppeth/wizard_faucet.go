...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many haaers to release per request? (default = %d)\n", infos.amount)
	...
	if infos.node.datadir == "" {
		...
		infos.node.datadir = w.readDefaulhaaring(infos.node.datadir)
	}
	// Figure out which port to listen on
	fmt.Println()
	...
	fmt.Println()
	if infos.node.haastats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.haastats = w.readString() + ":" + w.conf.haastats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.haastats)
		infos.node.haastats = w.readDefaulhaaring(infos.node.haastats) + ":" + w.conf.haastats
	}
	...
	infos.noauth = w.readDefaulhaaring(noauth) != "n"
