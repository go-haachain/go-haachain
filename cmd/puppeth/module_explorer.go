...
FROM pupphaa/explorer:latest

ADD haastats.json /haastats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../haa-net-intelligence-api && pm2 start /haastats.json)' >  explorer.sh && \
	echo '(cd ../haaerchain-light && npm start &)'                      >> explorer.sh && \
	echo '/parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerhaastats is the configuration file for the haastats javascript client.
var explorerhaastats = `[
  {
    "name"              : "node-app",
    "script"            : "app.js",
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.haaereum
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.haastats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	haastats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerhaastats)).Execute(haastats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.haastats[:strings.Index(config.haastats, ":")],
		"Secret": config.haastats[strings.Index(config.haastats, ":")+1 : strings.Index(config.haastats, "@")],
		"Host":   config.haastats[strings.Index(config.haastats, "@")+1:],
	})
	files[filepath.Join(workdir, "haastats.json")] = haastats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.haaereum"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		haastats: infos.envvars["STATS"],
	}
	return stats, nil
}
