...
FROM pupphaa/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'ghaa --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'ghaa --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --haastats \'{{.haastats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*"' >> wallet.sh

RUN \
	sed -i 's/PupphaaNetworkID/{{.NetworkID}}/g' dist/js/haaerwallet-master.js && \
	sed -i 's/PupphaaNetwork/{{.Network}}/g'     dist/js/haaerwallet-master.js && \
	sed -i 's/PupphaaDenom/{{.Denom}}/g'         dist/js/haaerwallet-master.js && \
	sed -i 's/PupphaaHost/{{.Host}}/g'           dist/js/haaerwallet-master.js && \
	sed -i 's/PupphaaRPCPort/{{.RPCPort}}/g'     dist/js/haaerwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whhaaer
// it's running, and if yes, whhaaer it's responsive.
