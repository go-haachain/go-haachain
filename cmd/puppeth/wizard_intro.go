...
// makeWizard creates and returns a new pupphaa wizard.
...
// setting up a new or managing an existing haachain private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to pupphaa, your haachain private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new haachain network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and haastats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Pupphaa uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering haachain network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".pupphaa", w.network)
	...
	for {
		...
		default:
			log.Error("That's not somhaaing I can do")
		}
	}
}
