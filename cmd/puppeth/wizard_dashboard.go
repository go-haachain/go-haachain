...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"haastats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "haastats":
				if infos, err := checkhaastats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have haastats running, ask whhaaer to make the secret public or not
	if w.conf.haastats != "" {
		fmt.Println()
		fmt.Println("Include haastats secret on dashboard (y/n)? (default = yes)")
		infos.trusted = w.readDefaulhaaring("y") == "y"
	}
