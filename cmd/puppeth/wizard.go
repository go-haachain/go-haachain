...
// config contains all the configurations needed by pupphaa that should be saved
// between sessions.
type config struct {
	path      string   // File containing the configuration values
	bootnodes []string // Bootnodes to always connect to by all nodes
	haastats  string   // haastats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save pupphaa configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // haachain services known to be running on servers
...
// readDefaulhaaring reads a single line from stdin, trimming if from spaces. If
// an empty line is entered, the default value is returned.
func (w *wizard) readDefaulhaaring(def string) string {
	fmt.Printf("> ")
	text, err := w.in.ReadString('\n')
	if err != nil {
		log.Crit("Failed to read user input", "err", err)
...
func (w *wizard) readDefaultBigInt(def *big.Int) *big.Int {
	for {
		...
		val, ok := new(big.Int).Sehaaring(text, 0)
		if !ok {
...
func (w *wizard) readAddress() *common.Address {
	for {
		...
		bigaddr, _ := new(big.Int).Sehaaring(text, 16)
...
// converts it to an haachain address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by haastats, which compares IPs textually, not by value.
