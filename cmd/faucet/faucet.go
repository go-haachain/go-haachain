...
// faucet is a haaer faucet backed by a light client.
package main
...
var (
	...
	haaPortFlag = flag.Int("haaport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the haachain protocol")
	statsFlag   = flag.String("haastats", "", "haastats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of haaers to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for haachain and the faucet")
)

var (
	haaer = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s haaers", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *haaPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // haachain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an haachain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // haachain protocol stack
	client *haaclient.Client   // Client connection to the haachain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	stack, err := node.New(&node.Config{
		...
		P2P: p2p.Config{
			...
			MaxPeers:         25,
			BoohaarapNodesV5: enodes,
		},
	})
	if err != nil {
		return nil, err
	}
	// Assemble the haachain light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := haa.DefaultConfig
	...
	// Assemble the haastats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.Lighthaachain
			ctx.Service(&serv)
			return haastats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := haaclient.NewClient(api)
...
// close terminates the haachain connection and tears down the faucet.
...
// apiHandler handles requests for haaer grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, haaer),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the haachain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Somhaaing funky happened, please open an issue at https://github.com/haachain/go-haachain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), haaer)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance = new(big.Int).Div(balance, haaer)
...
// the username, avatar URL and haachain address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for haachain addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No haachain address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the haachain address and profile URL.
...
// returning the username, avatar URL and haachain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain haachain address,
