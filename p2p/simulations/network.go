...
import (
	...
	"github.com/haachain/go-haachain/p2p/simulations/adapters"
)
...
// starting nodes and connecting them toghaaer.
//
...
// Connect connects two nodes toghaaer by calling the "admin_addPeer" RPC
...
// NOTE:
// it also checks whhaaer there has been recent attempt to connect the peers
...
type Node struct {
	...
	// Up tracks whhaaer or not the node is running
	Up bool `json:"up"`
}
...
type Conn struct {
	...
	// Up tracks whhaaer or not the connection is active
	Up bool `json:"up"`
...
// nodesUp returns whhaaer both nodes are currently up
