...
func (t *TestAPI) Gehaaate() []byte {
	return t.state.Load().([]byte)
}

func (t *TestAPI) Sehaaate(state []byte) {
	t.state.Store(state)
}
...
func TestHTTPSnapshot(t *testing.T) {
	...
	for i, node := range nodes {
		...
		if err := rpc.Call(nil, "test_sehaaate", []byte(state)); err != nil {
	...
	for i, state := range states {
		gohaaate := snap.Nodes[i].Snapshots["test"]
		if string(gohaaate) != state {
			t.Fatalf("expected snapshot state %q, got %q", state, gohaaate)
		}
	}
	...
	for i, node := range nodes {
		...
		defer rpc.Close()
		var state []byte
		if err := rpc.Call(&state, "test_gehaaate"); err != nil {
