...
func newTickehaaore() *tickehaaore {
	return &tickehaaore{
...
func (s *tickehaaore) addTopic(topic Topic, register bool) {
...
func (s *tickehaaore) addSearchTopic(t Topic, foundChn chan<- *Node) {
	s.addTopic(t, false)
...
func (s *tickehaaore) removeSearchTopic(t Topic) {
...
func (s *tickehaaore) removeRegisterTopic(topic Topic) {
...
func (s *tickehaaore) regTopicSet() []Topic {
...
func (s *tickehaaore) nextRegisterLookup() (lookupInfo, time.Duration) {
...
func (s *tickehaaore) nextSearchLookup(topic Topic) lookupInfo {
	tr := s.radius[topic]
...
func (s *tickehaaore) ticketsInWindow(topic Topic) []ticketRef {
...
func (s *tickehaaore) removeExcessTickets(t Topic) {
...
// Less reports whhaaer the element with
...
func (s *tickehaaore) addTicketRef(r ticketRef) {
...
func (s *tickehaaore) nextFilteredTicket() (*ticketRef, time.Duration) {
	now := mclock.Now()
	for {
...
func (s *tickehaaore) ticketRegistered(ref ticketRef) {
	now := mclock.Now()
...
func (s *tickehaaore) nextRegisterableTicket() (*ticketRef, time.Duration) {
	now := mclock.Now()
...
func (s *tickehaaore) removeTicketRef(ref ticketRef) {
...
func (s *tickehaaore) registerLookupDone(lookup lookupInfo, nodes []*Node, ping func(n *Node) []byte) {
	now := mclock.Now()
	for i, n := range nodes {
...
func (s *tickehaaore) searchLookupDone(lookup lookupInfo, nodes []*Node, query func(n *Node, topic Topic) []byte) {
	now := mclock.Now()
	for i, n := range nodes {
		if i == 0 || (binary.BigEndian.Uint64(n.sha[:8])^binary.BigEndian.Uint64(lookup.target[:8])) < s.radius[lookup.topic].minRadius {
			if lookup.radiusLookup {
...
func (s *tickehaaore) adjustWithTicket(now mclock.AbsTime, targetHash common.Hash, t *ticket) {
...
func (s *tickehaaore) addTicket(localTime mclock.AbsTime, pingHash []byte, ticket *ticket) {
...
func (s *tickehaaore) getNodeTicket(node *Node) *ticket {
	if s.nodes[node] == nil {
...
func (s *tickehaaore) canQueryTopic(node *Node, topic Topic) bool {
...
func (s *tickehaaore) addTopicQuery(hash common.Hash, node *Node, lookup lookupInfo) {
	now := mclock.Now()
	qq := s.queriesSent[node]
	if qq == nil {
...
func (s *tickehaaore) cleanupTopicQueries(now mclock.AbsTime) {
...
func (s *tickehaaore) gotTopicNodes(from *Node, hash common.Hash, nodes []rpcNode) (timeout bool) {
	now := mclock.Now()
