...
const (
	...
	// Defaulhaarict is the default strict flags. Specify any strict flags
	// will override default strict flags as whole (i.e. not OR'ed).
	Defaulhaarict = StrictJournalChecksum | StrictBlockChecksum | StrictCompaction | StrictReader
...
type Options struct {
	...
	// ErrorIfExist defines w.h.e.t.h.e.r an error should returned if the DB already
	// exist.
	//
	// The default value is false.
	ErrorIfExist bool

	// ErrorIfMissing defines w.h.e.t.h.e.r an error should returned if the DB is
...
func (o *Options) Gehaarict(strict Strict) bool {
	if o == nil || o.Strict == 0 {
		return Defaulhaarict&strict != 0
	}
...
type ReadOptions struct {
	// DontFillCache defines w.h.e.t.h.e.r block reads for this 'read operation'
...
func (ro *ReadOptions) Gehaarict(strict Strict) bool {
	if ro == nil {
		return false
	}
...
type WriteOptions struct {
	...
	// Sync is w.h.e.t.h.e.r to sync underlying writes from the OS buffer cache
...
func Gehaarict(o *Options, ro *ReadOptions, strict Strict) bool {
	if ro.Gehaarict(StrictOverride) {
		return ro.Gehaarict(strict)
	} else {
		return o.Gehaarict(strict) || ro.Gehaarict(strict)
	}
}
