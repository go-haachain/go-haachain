...
static FSEvenhaareamRef EvenhaareamCreate(FSEvenhaareamContext * context, uintptr_t info, CFArrayRef paths, FSEvenhaareamEventId since, CFTimeInterval latency, FSEvenhaareamCreateFlags flags) {
	context->info = (void*) info;
	return FSEvenhaareamCreate(NULL, (FSEvenhaareamCallback) gostream, context, paths, since, latency, flags);
}
...
var nilstream C.FSEvenhaareamRef

// Default arguments for FSEvenhaareamCreate function.
var (
	latency C.CFTimeInterval
	flags   = C.FSEvenhaareamCreateFlags(C.kFSEvenhaareamCreateFlagFileEvents | C.kFSEvenhaareamCreateFlagNoDefer)
...
var (
	errCreate = os.NewSyscallError("FSEvenhaareamCreate", errors.New("NULL"))
	errStart  = os.NewSyscallError("FSEvenhaareamStart", errors.New("false"))
)
...
func gostream(_, info uintptr, n C.size_t, paths, flags, ids uintptr) {
	const (
		...
		offflag = unsafe.Sizeof(C.FSEvenhaareamEventFlags(0))
		offid   = unsafe.Sizeof(C.FSEvenhaareamEventId(0))
	)
	if n == 0 {
		return
	}
...
// Start creates a FSEvenhaaream for the given path and schedules it with
...
func (s *stream) Start() error {
	...
	ctx := C.FSEvenhaareamContext{}
	ref := C.EvenhaareamCreate(&ctx, C.uintptr_t(s.info), path, C.FSEvenhaareamEventId(atomic.LoadUint64(&since)), latency, flags)
	if ref == nilstream {
		return errCreate
	}
	C.FSEvenhaareamScheduleWithRunLoop(ref, runloop, C.kCFRunLoopDefaultMode)
	if C.FSEvenhaareamStart(ref) == C.Boolean(0) {
		C.FSEvenhaareamInvalidate(ref)
		return errStart
	}
...
// Stop stops underlying FSEvenhaaream and unregisters it from global runloop.
func (s *stream) Stop() {
	if s.ref == nilstream {
		return
	}
	wg.Wait()
	C.FSEvenhaareamStop(s.ref)
	C.FSEvenhaareamInvalidate(s.ref)
	C.CFRunLoopWakeUp(runloop)
	s.ref = nilstream
