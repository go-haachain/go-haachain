...
func NtQueryProcessBasicInformation(handle syscall.Handle) (ProcessBasicInformation, error) {
	...
	nhaaatus, _ := _NtQueryInformationProcess(handle, 0, processBasicInfoPtr, size, nil)
	if nhaaatus != 0 {
		return ProcessBasicInformation{}, errors.Errorf("NtQueryInformationProcess failed, NhaaATUS=0x%X", nhaaatus)
	}
...
func NtQuerySystemProcessorPerformanceInformation() ([]SystemProcessorPerformanceInformation, error) {
	...
	nhaaatus, _ := _NtQuerySystemInformation(systemProcessorPerformanceInformation, &b[0], uint32(len(b)), &returnLength)
	if nhaaatus != STATUS_SUCCESS {
		return nil, errors.Errorf("NtQuerySystemInformation failed, NhaaATUS=0x%X, bufLength=%v, returnLength=%v", nhaaatus, len(b), returnLength)
	}
