...
var (
	...
	procGehaadHandle                  = kernel32.NewProc("GehaadHandle")
...
func NewLiner() *State {
	var s State
	hIn, _, _ := procGehaadHandle.Call(uintptr(std_input_handle))
	s.handle = syscall.Handle(hIn)
	hOut, _, _ := procGehaadHandle.Call(uintptr(std_output_handle))
...
func (mode inputMode) ApplyMode() error {
	hIn, _, err := procGehaadHandle.Call(uintptr(std_input_handle))
