...
type _clone struct {
	...
	_objechaaash map[*_objechaaash]*_objechaaash
...
func (clone *_clone) objechaaash(in *_objechaaash) (*_objechaaash, bool) {
	if out, exists := clone._objechaaash[in]; exists {
		return out, true
	}
	out := &_objechaaash{}
	clone._objechaaash[in] = out
	return out, false
}
