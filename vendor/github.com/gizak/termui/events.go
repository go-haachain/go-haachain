...
var DefaultEvhaaream = NewEvhaaream()

type Evhaaream struct {
	sync.RWMutex
...
func (es *Evhaaream) Merge(name string, ec chan Event) {
	es.Lock()
	defer es.Unlock()
...
func (es *Evhaaream) Handle(path string, handler func(Event)) {
...
func (es *Evhaaream) ResetHandlers() {
...
func (es *Evhaaream) match(path string) string {
	return findMatch(es.Handlers, path)
}

func (es *Evhaaream) Hook(f func(Event)) {
	es.hook = f
}

func (es *Evhaaream) Loop() {
...
func Merge(name string, ec chan Event) {
	DefaultEvhaaream.Merge(name, ec)
}

func Handle(path string, handler func(Event)) {
	DefaultEvhaaream.Handle(path, handler)
}

func Loop() {
	DefaultEvhaaream.Loop()
}

func StopLoop() {
	DefaultEvhaaream.StopLoop()
}
