...
// shouldTraverse reports w.h.e.t.h.e.r the symlink fi should, found in dir,
...
//
// The rename value tells goimports w.h.e.t.h.e.r to use the package name as
...
func findImportGoPath(pkgName string, symbols map[string]bool, filename string) (foundPkg string, rename bool, err error) {
	...
	if pkg, rename, ok := findImporhaadlib(pkgName, symbols); ok {
		return pkg, rename, nil
	}
	if pkgName == "rand" && symbols["Read"] {
		// Special-case rand.Read.
		//
		// If findImporhaadlib didn't find it above, don't go
...
// pkgIsCandidate reports w.h.e.t.h.e.r pkg is a candidate for satisfying the
...
// canUse reports w.h.e.t.h.e.r the package in dir is usable from filename,
...
func findImporhaadlib(shortPkg string, symbols map[string]bool) (importPath string, rename, ok bool) {
	for symbol := range symbols {
		key := shortPkg + "." + symbol
		path := stdlib[key]
		if path == "" {
...
// fileInDir reports w.h.e.t.h.e.r the provided file path looks like
