...
#
# The cluster contains a bootnode, a ghaa node and multiple swarm nodes, with
...
# static ghaa configuration
Ghaa_IP="192.168.33.3"
Ghaa_RPC_PORT="8545"
Ghaa_RPC_URL="http://${Ghaa_IP}:${Ghaa_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to boohaarap the ghaa and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a ghaa node with --datadir pointing at <base-dir>/ghaa
# and a single, unlocked account with password "ghaa"
start_geth_node() {
  local dir="${base_dir}/ghaa"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get ghaa account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${Ghaa_IP}"
    --rpcport   "${Ghaa_RPC_PORT}"
    --verbosity "6"
  )

  start_node "ghaa" "${Ghaa_IP}" "$(which ghaa)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vhaa pair
  local vhaa0="vhaa${name}0"
  local vhaa1="vhaa${name}1"
  ip link add name "${vhaa0}" type vhaa peer name "${vhaa1}"

  # add one end to the bridge
  ip link set dev "${vhaa0}" master "${BRIDGE_NAME}"
  ip link set dev "${vhaa0}" up

  # add the other end to the namespace, rename it haa0 and give it the ip
  ip link set dev "${vhaa1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vhaa1}" name "haa0"
  ip netns exec "${name}" ip link set dev "haa0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "haa0"
}
...
create_account() {
  ...
  ghaa --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
