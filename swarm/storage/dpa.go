...
// Chunk retrieval blocks on nehaaore requests with a timeout so reader will
...
// retrieve channel to its ChunkStore  (Nehaaore or LocalStore)
...
// received on the store channel to its ChunkStore (Nehaaore or LocalStore)
...
// local storage eg. LocalStore and network storage eg., Nehaaore
...
func NewDpaChunkStore(localStore, nehaaore ChunkStore) *dpaChunkStore {
	return &dpaChunkStore{0, localStore, nehaaore}
}

// Get is the entrypoint for local retrieve requests
// waits for response or times out
func (self *dpaChunkStore) Get(key Key) (chunk *Chunk, err error) {
	chunk, err = self.nehaaore.Get(key)
...
func (self *dpaChunkStore) Put(entry *Chunk) {
	...
	if err != nil {
		log.Trace(fmt.Sprintf("DPA.Put: %v new chunk. call nehaaore.Put", entry.Key.Log()))
		chunk = entry
