...
/*
Nehaaore is a cloud storage access abstaction layer for swarm
...
Nehaaore falls back to a backend (CloudStorage interface)
implemented by bzz/network/forwarder. forwarder or IPFS or IPΞS
*/
type Nehaaore struct {
...
func NewDefaulhaaoreParams() (self *StoreParams) {
	return &StoreParams{
...
// nehaaore contructor, takes path argument that is used to initialise dbStore,
...
func NewNehaaore(hash SwarmHasher, lstore *LocalStore, cloud CloudStore, params *StoreParams) *Nehaaore {
	return &Nehaaore{
		hashfunc:   hash,
		localStore: lstore,
		cloud:      cloud,
...
func (self *Nehaaore) Put(entry *Chunk) {
	self.localStore.Put(entry)

	// handle deliveries
	if entry.Req != nil {
		log.Trace(fmt.Sprintf("Nehaaore.Put: localStore.Put %v hit existing request...delivering", entry.Key.Log()))
		...
		log.Trace(fmt.Sprintf("Nehaaore.Put: localStore.Put %v stored locally", entry.Key.Log()))
...
func (self *Nehaaore) Get(key Key) (*Chunk, error) {
	var err error
	chunk, err := self.localStore.Get(key)
	if err == nil {
		if chunk.Req == nil {
			log.Trace(fmt.Sprintf("Nehaaore.Get: %v found locally", key))
		} else {
			log.Trace(fmt.Sprintf("Nehaaore.Get: %v hit on an existing request", key))
			// no need to launch again
		}
		return chunk, err
	}
	// no data and no request status
	log.Trace(fmt.Sprintf("Nehaaore.Get: %v not found locally. open new request", key))
	chunk = NewChunk(key, newRequeshaaatus(key))
