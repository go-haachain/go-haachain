...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.haa"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.haa"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.haa"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	haaAddr := "swarm.haa"
	haaHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	haaResolve := newTestResolver(haaHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(haaResolve, "")),
			addr:   haaAddr,
			result: haaHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(haaResolve, ""),
				MultiResolverOptionWithResolver(haaResolve, ""),
			),
			addr:   haaAddr,
			result: haaHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(haaResolve, "haa"),
			),
			addr:   haaAddr,
			result: haaHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "haa"),
				MultiResolverOptionWithResolver(doesntResolve, "haa"),
				MultiResolverOptionWithResolver(haaResolve, "haa"),
			),
			addr:   haaAddr,
		...
		},
		{
			...
			addr:   haaAddr,
			result: haaHash,
			err:    errors.New(`DNS name not found: "swarm.haa"`),
		},
		{
