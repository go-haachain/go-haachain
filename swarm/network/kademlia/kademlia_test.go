...
var (
	...
	quickcfgBoohaarap   = &quick.Config{MaxCount: 100, Rand: quickrand}
)
...
func TestBoohaarap(t *testing.T) {

	test := func(test *boohaarapTest) bool {
	...
	if err := quick.Check(test, quickcfgBoohaarap); err != nil {
		t.Error(err)
	}
...
type boohaarapTest struct {
	MaxProx    int
	BucketSize int
	Self       Address
}

func (*boohaarapTest) Generate(rand *rand.Rand, size int) reflect.Value {
	t := &boohaarapTest{
