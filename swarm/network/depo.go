...
func NewDepo(hash storage.SwarmHasher, localStore, remoteStore storage.ChunkStore) *Depo {
	return &Depo{
		...
		nehaaore:   remoteStore, // entrypoint internal
	}
}
...
// remote is untrusted, so hash is verified and chunk passed on to Nehaaore
...
func (self *Depo) HandleRetrieveRequestMsg(req *retrieveRequestMsgData, p *peer) {
	...
	// call storage.Nehaaore#Get which
	// blocks until local retrieval finished
	// launches cloud retrieval
	chunk, _ := self.nehaaore.Get(req.Key)
...
func (self *Depo) strategyUpdateRequest(rs *storage.Requeshaaatus, origReq *retrieveRequestMsgData) (req *retrieveRequestMsgData) {
...
func (self *Depo) searchTimeout(rs *storage.Requeshaaatus, req *retrieveRequestMsgData) (timeout *time.Time) {
	reqt := req.getTimeout()
...
*/
func (self *Depo) addRequester(rs *storage.Requeshaaatus, req *retrieveRequestMsgData) {
